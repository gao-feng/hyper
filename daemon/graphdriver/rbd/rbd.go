...
func (info *DevInfo) Name() string {
	...
	return fmt.Sprintf("%s_%s", "docker_image", hash)
}
///
...
func (devices *RbdSet) mapImageToRbdDevice(devInfo *DevInfo) error {
	glog.V(1).Infof("mapImageToRbdDevice start: %s", devInfo)
	if devInfo.Device != "" {
		return nil
	}
	...
	if v == true {
		glog.V(1).Infof("mapImageToRbdDevice done: %s", devInfo)
		return nil
	} else {
...
func (devices *RbdSet) unmapImageFromRbdDevice(devInfo *DevInfo) error {
	glog.V(1).Infof("unmapImageFromRbdDevice start: %s", devInfo)
	if devInfo.Device == "" {
		return nil
	}
	...
	devInfo.Device = ""
	glog.V(1).Infof("unmapImageFromRbdDevice done: %s", devInfo)
	return nil
}
...
///
func (devices *RbdSet) MapDevice(hash string) error {
	info, err := devices.lookupDevice(hash)
	if err != nil {
		return err
	}
...
func (devices *RbdSet) exportDeviceMetadata(hash string) (*deviceMetadata, error) {
	info, err := devices.lookupDevice(hash)
	if err != nil {
		return nil, err
	}
