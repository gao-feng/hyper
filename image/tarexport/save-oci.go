...
func getRefs(irefs map[string]string) (map[string]string, map[string]reference.NamedTagged, error) {
	refs := make(map[string]string)
	reversed := make(map[string]reference.NamedTagged)
	for image, ref := range irefs {
		...
		tagged, err := reference.WithTag(r, reference.DefaultTag)
		if err != nil {
			return nil, nil, err
		}
...
func (l *tarexporter) parseOCINames(names []string, irefs map[string]string) (map[image.ID]*imageDescriptor, error) {
	refs, _, err := getRefs(irefs)
	if err != nil {
		return nil, err
	}
	...
	for _, name := range names {
		...
		if reference.IsNameOnly(ref) {
			...
			for _, assoc := range assocs {
				if err := addAssoc(assoc.ImageID, assoc.Ref); err != nil {
					return nil, err
				}
			}
			if len(assocs) == 0 {
		...
		if err := addAssoc(id, ref); err != nil {
			return nil, err
		}
	}
	return imgDescr, nil
...
func (s *ociSaveSession) save(outStream io.Writer) error {
	...
	for id, info := range s.images {
		...
		if len(info.refs) == 0 {
			// TODO(runcom): handle foreign srcs like save.go
			if err := s.saveImage(id, digest.Digest(id).Hex()); err != nil {
				return err
			}
		}
	}
...
func (s *ociSaveSession) saveImage(id image.ID, ref string) error {
	...
	configFile, err := blobPath(s.outDir, digest.Digest(id))
	if err != nil {
		return err
	}
